...
{{- $align := .Get 0 -}}
{{- $validAligns := slice "left" "center" "right" "justify" "start" "end" -}}
{{- if in $validAligns $align -}}
  <p style="text-align:{{ $align }}">{{ .Get 1 | markdownify }}</p>
{{- else -}}
...
`notice` 和 `simple-notice` 的模板逻辑基本相同，只是 CSS 类名前缀不同。为了更好地维护，这里抽取了一个公共 partial `notice-base.html`，两个 shortcode 分别调用它。

首先创建公共 partial `notice-base.html`，内容如下：

```html
<!-- 文件位置：~/layouts/partials/notice-base.html -->

<!--https://github.com/martignoni/hugo-notice-->
{{- $ctx := .ctx -}}
{{- $classPrefix := .classPrefix -}}
{{- $noticeType := $ctx.Get 0 -}}

{{- $raw := (markdownify $ctx.Inner | chomp) -}}

{{- $block := findRE "(?is)^<(?:address|article|aside|blockquote|canvas|dd|div|dl|dt|fieldset|figcaption|figure|footer|form|h(?:1|2|3|4|5|6)|header|hgroup|hr|li|main|nav|noscript|ol|output|p|pre|section|table|tfoot|ul|video)\\b" $raw 1 -}}

{{ $icon := (replace (index $ctx.Site.Data.SVG $noticeType) "icon" (printf "icon %s-icon" $classPrefix)) }}
<div class="{{ $classPrefix }} {{ $noticeType }}" {{ if len $ctx.Params | eq 2 }} id="{{ $ctx.Get 1 }}" {{ end }}>
    <div class="{{ $classPrefix }}-title">{{ $icon | safeHTML }}</div>
    {{- if or $block (not $raw) }}{{ $raw }}{{ else }}<p>{{ $raw }}</p>{{ end -}}
</div>
```

然后创建 `notice.html` 简码，只需一行：

```html
<!-- 文件位置：~/layouts/shortcodes/notice.html -->

{{- $_ := .Inner -}}{{ partial "notice-base.html" (dict "ctx" . "classPrefix" "notice") }}
```

然后添加自定义 CSS 样式：

```scss
// 文件位置：~/assets/scss/custom/_shortcodes.scss（由 _custom.scss 引入）

$notice-variants: (
    'warning': (color: hsl(0, 65%, 65%), bg: hsla(0, 65%, 65%, 0.15)),
    'info':    (color: hsl(30, 80%, 70%), bg: hsla(30, 80%, 70%, 0.15)),
    'note':    (color: hsl(200, 65%, 65%), bg: hsla(200, 65%, 65%, 0.15)),
    'tip':     (color: hsl(140, 65%, 65%), bg: hsla(140, 65%, 65%, 0.15)),
);
$notice-dark-variants: (
    'warning': (color: hsl(0, 25%, 35%), bg: hsla(0, 25%, 35%, 0.15)),
    'info':    (color: hsl(30, 25%, 35%), bg: hsla(30, 25%, 35%, 0.15)),
    'note':    (color: hsl(200, 25%, 35%), bg: hsla(200, 25%, 35%, 0.15)),
    'tip':     (color: hsl(140, 25%, 35%), bg: hsla(140, 25%, 35%, 0.15)),
);

.notice {
    font-family: 'glyph-correction', 'LXGW WenKai TC', 'Noto Serif SC', serif;
    position: relative;
    ...
    @each $name, $c in $notice-variants {
        &.notice-#{$name} {
            background: map-get($c, bg);
            border-left: 5px solid map-get($c, color);
            .notice-title { color: map-get($c, color); }
        }
    }
}

[data-theme="dark"] .notice {
    @each $name, $c in $notice-dark-variants {
        &.notice-#{$name} {
            background: map-get($c, bg);
            border-left: 5px solid map-get($c, color);
            .notice-title { color: map-get($c, color); }
...
`simple-notice` 简码同样只需一行，调用同一个公共 partial：

```html
<!-- 文件位置：~/layouts/shortcodes/simple-notice.html -->

{{- $_ := .Inner -}}{{ partial "notice-base.html" (dict "ctx" . "classPrefix" "simple-notice") }}
```
...
$simple-notice-variants: (
    'warning': hsl(0, 100%, 35%),
    'info':    hsl(40, 80%, 45%),
    'note':    hsl(210, 100%, 25%),
    'tip':     hsl(150, 100%, 25%),
);
$simple-notice-dark-variants: (
    'warning': hsl(0, 65%, 65%),
    'info':    hsl(30, 80%, 70%),
    'note':    hsl(200, 65%, 65%),
    'tip':     hsl(140, 65%, 65%),
...
.simple-notice {
    ...
    @each $name, $color in $simple-notice-variants {
        &.simple-notice-#{$name} {
            border-top: 2px solid $color;
            color: $color;
            .simple-notice-title { color: $color; }
            a {
                color: $color;
...
[data-theme="dark"] .simple-notice {
    @each $name, $color in $simple-notice-dark-variants {
        &.simple-notice-#{$name} {
            border-top: 2px solid $color;
            color: $color;
            .simple-notice-title { color: $color; }
...
这个简码是用来替代 Markdown 默认的引用样式。`quote` 和 `quote-center` 共享同一段 `<p>` 标签包裹逻辑，因此抽取出公共 partial `quote-content.html`：

```html
<!-- 文件位置：~/layouts/partials/quote-content.html -->
...
然后创建 `quote.html` 简码，调用公共 partial：
...
<blockquote class="quote{{ range .Params }} {{ . }}{{ end }}">
    ...
    {{ partial "quote-content.html" (dict "content" $content) }}
</blockquote>
```
...
这是一个用于居中引用的简码，其样式也是借鉴自 Hexo 的 NexT 主题。`quote-center.html` 同样调用公共 partial `quote-content.html`：
