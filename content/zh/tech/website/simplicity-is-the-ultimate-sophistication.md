...
Waline v3 相较于 v2，最大的变动是采用了 `module` 的方式，从 `waline.js` 中调用了 `init`。因此在修改主题的时候需要注意，如果在 `module` 中定义了加载评论的函数 `loadComments`，该函数无法直接在其他区域直接被调用。
...
此外，目前评论的邮件提醒功能异常，我还没有检查是哪个环节的问题。现在正忙着补发这两年构思好但没发布的文章，已无再多时间找 BUG。至于何时能修好，还是个未知数。
