...
### 侧边栏（Next Muse 风格）
- 左侧滑入式文章目录（TOC），仅 TOC 开启的页面显示
- `enableSidebar` 全局开关，三横线汉堡图标触发
- 宽屏推挤主内容（1200px+），窄屏叠加遮罩
- body 与 header 同步位移，resize 自动冻结过渡防抖动
- CSS 计数器三级编号（1 / 1.1 / 1.1.1），IntersectionObserver 高亮当前章节
- 按钮动画与导航栏汉堡图标一致
